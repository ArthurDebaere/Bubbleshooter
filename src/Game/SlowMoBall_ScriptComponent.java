...
public class SlowMoBall_ScriptComponent extends PowerBall_ScriptComponent {
    private GameObject gameObject;
    private GameObjectManager gameObjectManager;

    public void initialize(GameObject gameObject) {
        super.initialize(gameObject);
        String powerName = getSettings().getPowerballs().get(2).getPowerName();
    ...
    public void update(double time) {
        if(getIsFired()) {
            ...
            gameObjectManager.getBubbleField(getPlayerNumber()).setNumberOfBallsAddedForAddingNewRow(25, true);
            setIsFired(false);
